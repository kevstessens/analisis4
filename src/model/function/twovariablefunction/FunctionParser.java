...
public class FunctionParser {
    ...
    public TwoVariableFunction getFunctionFromMathExpression(String mathExpression) throws NonValidExpressionError {
        variableX = new Variable("x");
        variableY = new Variable("y");
        return new TwoVariableFunctionImplementation(variableX, variableY,
                getOperableElementFormMathExpression(mathExpression)) {
        };
    }

    private OperableElement getOperableElementFormMathExpression(String mathExpression) throws NonValidExpressionError {
        ...
        for (int i = 0; i < mathExpression.length(); i++) {
            ...
            if ((actualChar < '(' || actualChar > '9' || actualChar == '´') && actualChar != ' ' && actualChar != 'x'
                    && actualChar != 'y' && actualChar != '^' && actualChar != 'l' && actualChar != 's'
                    && actualChar != 'c' && actualChar != 't') {
            ...
            switch (actualChar) {
                ...
                case (' '):
                    break;
                case ('x'):
                    result.push(variableX);
                    break;
                case ('y'):
                    result.push(variableY);
                    break;
                case ('l'):
